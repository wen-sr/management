      ...
      Map<String, Object> context, WxMpService wxMpService,
        ...
        if (StringUtils.startsWithAny(wxMessage.getContent(), "B", "b")) {
            ...
            if(stockMiniload == null) {
                content = "【" + wxMessage.getContent() + "】该箱号未分配储位";
                return new TextBuilder().build(content, wxMessage, weixinService);
            }
            if(StringUtils.isBlank(stockMiniload.getContainerCode())){
                content = "您要查询箱号【" + wxMessage.getContent() + "】尚未到位，分配的储位是："+ stockMiniload.getBankId() +"号立库"+ stockMiniload.getBankLayer() +"层"+ stockMiniload.getBankColumn();
                return new TextBuilder().build(content, wxMessage, weixinService);
            }
            if(StringUtils.isNotBlank(stockMiniload.getContainerCode())){
                content = "您要查询箱号【" + wxMessage.getContent() + "】已到位，分配的储位是："+ stockMiniload.getBankId() +"号立库"+ stockMiniload.getBankLayer() +"层"+ stockMiniload.getBankColumn();
                return new TextBuilder().build(content, wxMessage, weixinService);
            }
        }
